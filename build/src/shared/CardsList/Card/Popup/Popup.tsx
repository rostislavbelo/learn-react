...
export function assoc<K extends string, T extends string>(key: K, text: T) {
  return <O extends Object>(obj: O) => ({...obj, [key]: text}) as K extends keyof O ? (Omit<O, K> & Record<K, T>) : (O & Record<K, T>) // необязательный хвост
}
...
const LIST = [
  {As: 'li' as const, text: 'Комментарии', className: `${styles.popupItem}`, dataMobile: 'hidden', icon: 'IconComment' as const, size: '15' as const},
  {As: 'li' as const, text: 'Поделиться', className: `${styles.popupItem}`, dataMobile: 'hidden', icon: 'IconShare' as const, size: '14' as const},
  {As: 'li' as const, text: 'Скрыть', className: `${styles.popupItem}`, dataMobile: 'visible', icon: 'IconHidden' as const, size: '14' as const},
  {As: 'li' as const, text: 'Сохранить', className: `${styles.popupItem}`, dataMobile: 'hidden', icon: 'IconSave' as const, size: '14' as const},
  {As: 'li' as const, text: 'Пожаловаться', className: `${styles.popupItem}`, dataMobile: 'visible', icon: 'IconComplain' as const, size: '16' as const},
  {As: 'li' as const, text: 'Закрыть', className: `${styles.popupItem}`, dataMobile: 'visible'},
...
export function Popup() {
  ...
  return (
    <ul className={styles.popupList}>
      <GenerateList list = {list.map((item)=>({...item, onClick: () => handleItemClick(item.id)}))} />
    </ul>
  );
}
